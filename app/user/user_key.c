...
//key_para_t *pkeys[USER_KEY_NUM];
//key_list_t key_list;

key_para_t * ICACHE_FLASH_ATTR user_key_init_single( uint8_t gpio_id, uint8_t gpio_func, uint32_t gpio_name,
...
void ICACHE_FLASH_ATTR user_key_set_long_count( key_para_t *pkey, uint16_t count )
{
	if ( pkey != NULL )
	{
...
LOCAL void ICACHE_FLASH_ATTR user_key_20ms_cb( void *arg )
{
...
void ICACHE_FLASH_ATTR user_key_init_list( key_list_t *plist )
{
	uint8_t i;
	if ( plist == NULL )
	{
